...
#[path = "../knowledge/architecture.rs"] mod architecture;
...
Architecture { #[arg(long, default_value = ".codanna/knowledge.json")] graph: PathBuf, #[arg(long, default_value_t=20)] hubs: usize }
}
...
Action::Architecture { graph, hubs } => { let graph=knowledge::io::load(&graph)?; serde_json::to_value(architecture::analyze(&graph,hubs)?)? }

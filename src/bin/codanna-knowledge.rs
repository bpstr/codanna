...
use clap::{Parser, Subcommand}; use std::path::PathBuf; use std::io::Write;
...
fn run() -> knowledge::Result<()> { let result = match Cli::parse().command {
...
}; let mut out=std::io::stdout().lock(); serde_json::to_writer(&mut out,&result)?; writeln!(out)?; Ok(()) }

...
impl WorkspaceRegistry {
        ...
        with_registry(&self.path, |registry| {
            if let Some(id) = id_for_root(registry, &root)? {
                ...
                if name.is_some_and(|name| name != existing.name) {
                    return Err(failure(
                        "Workspace is already registered under a different alias. Use 'codanna workspace rename' explicitly.",
                    ));
                }
    ...
    pub fn rename(&self, selector: &str, name: &str) -> Result<Workspace, IndexError> {
        ...
        with_registry(&self.path, |registry| {
            ...
            let info = registry
                .projects
                .get_mut(&id)
                .ok_or_else(|| failure("Workspace disappeared"))?;
            info.name = name.to_owned();
    ...
    pub fn relocate(&self, selector: &str, path: &Path) -> Result<Workspace, IndexError> {
        let root = discover(path)?
            .ok_or_else(|| failure("New location has no workspace configuration"))?;
        let settings = read_settings(&root)?;
        confined_index_path(&root, &settings)?;
        with_registry(&self.path, |registry| {
            let id = resolve_id(registry, selector)?;
            ...
            if old != &root
                && old.try_exists().map_err(|source| IndexError::FileRead {
                    path: old.clone(),
                    source,
                })?
            {
                return Err(failure(
                    "Original workspace still exists. A copy or another worktree needs a separate registration.",
                ));
            }
            if id_for_root(registry, &root)?.is_some_and(|other| other != id) {
                return Err(failure(
                    "New location belongs to another registered workspace",
                ));
            }
            let info = registry
                .projects
...
pub fn discover(start: &Path) -> Result<Option<PathBuf>, IndexError> {
    ...
    let directory = if canonical.is_dir() {
        ...
        canonical
            .parent()
            .ok_or_else(|| failure("Path has no parent directory"))?
    };
...
pub fn read_settings(root: &Path) -> Result<Settings, IndexError> {
    ...
    if real_config != config {
        return Err(failure(
            "Workspace configuration must be local, not a symlink to another configuration",
        ));
    }
    ...
    if let Some(configured) = &settings.workspace_root {
        let configured =
            fs::canonicalize(root.join(configured)).map_err(|source| IndexError::FileRead {
                path: root.join(configured),
                source,
            })?;
        if configured != root {
            return Err(failure(
                "Configuration workspace_root points to another workspace. Correct it before selecting this workspace.",
            ));
        }
    }
    Ok(settings)
...
pub fn confined_index_path(root: &Path, settings: &Settings) -> Result<PathBuf, IndexError> {
    ...
    {
        return Err(failure(
            "Workspace index_path cannot contain parent traversal",
        ));
    }
        ...
        match fs::symlink_metadata(&existing) {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
                let component = existing
                    .file_name()
                    .ok_or_else(|| failure("Invalid index path"))?
                    .to_owned();
    ...
    if resolved == state || !resolved.starts_with(&state) {
        return Err(failure(
            "Selected workspaces require a project-local index beneath .codanna. External/shared index paths are not supported by --workspace yet.",
        ));
    }
...
pub(super) fn id_for_root(
    ...
    match ids.as_slice() {
        ...
        [id] => Ok(Some(id.clone())),
        _ => Err(failure(
            "Multiple registry entries refer to this workspace root. Remove duplicates by ID before continuing.",
        )),
    }
}
...
fn resolve_id(registry: &ProjectRegistry, selector: &str) -> Result<String, IndexError> {
    ...
    let (id, _) = matches.next().ok_or_else(|| {
        failure(format!(
            "Workspace '{selector}' is not registered. Use 'codanna workspace list'."
        ))
    })?;
    if matches.next().is_some() {
        return Err(failure(format!(
            "Workspace alias '{selector}' is ambiguous. Select an exact ID from 'codanna workspace list'."
        )));
    }
    Ok(id.clone())
}
...
fn validate_alias(name: &str) -> Result<(), IndexError> {
    ...
    {
        return Err(failure(
            "Workspace aliases must be 1-64 ASCII letters, digits, '.', '_' or '-', starting with a letter or digit. Supply --name explicitly.",
        ));
    }
    Ok(())
}
...
fn ensure_alias_available(
    ...
    if registry
        .projects
        .iter()
        .any(|(id, info)| Some(id.as_str()) != own_id && (info.name == name || id == name))
    {
        return Err(failure(format!(
            "Workspace alias '{name}' is already in use. Choose a distinct --name."
        )));
    }
    Ok(())
}

...
fn fixture(source: &str, max_input: usize) -> (tempfile::TempDir, Settings, TcpListener) {
    ...
    settings.semantic_search.remote_url =
        Some(format!("http://{}", endpoint.local_addr().unwrap()));
...
fn runtime_inputs(settings: &Settings) -> (Vec<String>, usize) {
    ...
    let mut retained = 0;
    for file in FileWalker::new(options)
        .snapshot(
        ...
        for symbol in parser.parse(file).unwrap().raw_symbols {
            if let Some(source) = symbol.embedding_source {
                ...
                for fragment in &source.fragments {
                    assert_eq!(
                        bytes.get(fragment.range.clone()),
...
fn identity(settings: &Settings, policy: CodeEmbeddingPolicy) -> String {
    ...
    policy.bind_identity(backend_identity(
        "remote",
        "offline-body-fixture",
        Some(&endpoint),
        None,
        &budget,
    ))
...
fn body_plan_matches_runtime_segments_bytes_and_cache_after_source_edits() {
    ...
    assert_eq!(
        report.embedding_input_bytes,
        Some(inputs.iter().map(String::len).sum())
    );
    ...
    std::fs::write(
        ...
        source.replace("work()", "changed_work()"),
    )
    .unwrap();
...
fn body_plan_file_budget_exhaustion_remains_visible() {
    ...
    assert_eq!(
        report.body_sources + report.missing_body_sources,
...
fn body_plan_frozen_repository_inputs_match_the_runtime_preparation() {
    const ORACLE: &str =
        include_str!("../contributing/retrieval/evaluations/repository-tasks.json");
    ...
    for (path, source) in [
        (
            ...
            include_str!("../tests/fixtures/repository_task_sources/read.rs.fixture"),
        ),
        (
            "src/indexing/pipeline/stages/discover.rs",
            include_str!("../tests/fixtures/repository_task_sources/discover.rs.fixture"),
        ),
        (
            "src/mcp/tools/recall.rs",
            include_str!("../tests/fixtures/repository_task_sources/recall.rs.fixture"),
        ),
        (
            "src/embedding_cache.rs",
            include_str!("../tests/fixtures/repository_task_sources/embedding_cache.rs.fixture"),
        ),
        (
            "src/memory.rs",
            include_str!("../tests/fixtures/repository_task_sources/memory.rs.fixture"),
        ),
    ] {

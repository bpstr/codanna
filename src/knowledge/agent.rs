...
impl AnalysisIndex {
    pub fn new(graph: &Graph) -> Self {
        ...
        Self { incoming, outgoing, by_file }
    }

    pub fn impact(&self, graph: &Graph, request: &ImpactRequest) -> Result<ImpactReport> {
        if request.files.is_empty() || request.files.len() > 128 || request.max_depth > 8 || !(1..=512).contains(&request.max_nodes) {
            return Err("impact limits: files 1..128, depth 0..8, nodes 1..512".into());
        }
        for file in &request.files { validate_path(file)?; }
        if !graph.repositories.contains_key(&request.repo) { return Err("unknown repository".into()); }
        ...
        let mut queue: VecDeque<(String, usize)> = seed_ids.iter().cloned().map(|id| (id, 0)).collect();
        let mut visited: BTreeMap<String, usize> = seed_ids.iter().cloned().map(|id| (id, 0)).collect();
        ...
        while let Some((id, depth)) = queue.pop_front() {
            ...
            for edge_index in self.incoming.get(&id).into_iter().flatten() {
                examined += 1;
                if examined > 100_000 { truncated = true; break; }
                let edge = &graph.edges[*edge_index];
                if matches!(edge.relation.as_str(), "defines" | "contains") { continue; }
                let next = edge.from.clone();
                if visited.contains_key(&next) { continue; }
                if visited.len() >= request.max_nodes { truncated = true; continue; }
                let next_depth = depth + 1;
                visited.insert(next.clone(), next_depth);
                reasons.insert(next.clone(), format!("depends on changed evidence via {}", edge.relation));
                queue.push_back((next, next_depth));
            }
            if examined > 100_000 { break; }
        }
        ...
        let mut impacted: Vec<_> = visited
            ...
            .map(|(id, depth)| ImpactItem {
                ...
                depth: *depth,
                reason: reasons.get(id).cloned().unwrap_or_else(|| "connected impact".into()),
            })
            .collect();
        impacted.sort_by(|a, b| a.depth.cmp(&b.depth).then_with(|| a.node.id.cmp(&b.node.id)));

        let cross_repo = impacted.iter().filter(|item| item.node.source.repo != request.repo).count();
        ...
        let risk = if score >= 40 || cross_repo >= 3 { "high" } else if score >= 12 || cross_repo > 0 { "medium" } else { "low" };
    ...
    pub fn dead_code(&self, graph: &Graph, request: &DeadCodeRequest) -> Result<DeadCodeReport> {
        if request.limit == 0 || request.limit > 1000 { return Err("dead-code limit must be 1..1000".into()); }
        if request.repo.as_ref().is_some_and(|repo| !graph.repositories.contains_key(repo)) { return Err("unknown repository".into()); }
        let mut candidates = Vec::new();
        for node in graph.nodes.values() {
            if node.kind != Kind::Symbol { continue; }
            if request.repo.as_ref().is_some_and(|repo| &node.source.repo != repo) { continue; }
            let label = node.label.rsplit("::").next().unwrap_or(&node.label);
            if is_common_entrypoint(label, &node.source.path) { continue; }
            let incoming = self.incoming.get(&node.id).into_iter().flatten().filter(|edge_index| {
                let edge = &graph.edges[**edge_index];
                !matches!(edge.relation.as_str(), "defines" | "contains")
            }).count();
            if incoming != 0 { continue; }
            let outgoing = self.outgoing.get(&node.id).into_iter().flatten().filter(|edge_index| {
                !matches!(graph.edges[**edge_index].relation.as_str(), "defines" | "contains")
            }).count();
        ...
        candidates.sort_by(|a, b| b.outgoing_edges.cmp(&a.outgoing_edges).then_with(|| a.node.id.cmp(&b.node.id)));
...
fn is_common_entrypoint(label: &str, path: &str) -> bool {
    matches!(label, "main" | "init" | "run" | "start" | "register" | "bootstrap")
        || path.ends_with("/main.rs") || path.ends_with("/main.go") || path.ends_with("/__init__.py")
        || path.ends_with("/index.ts") || path.ends_with("/index.js")
}
...
fn impact_depth() -> usize { 3 }
fn impact_nodes() -> usize { 200 }

#[derive(Debug, Clone, Serialize)]
pub struct ImpactItem { pub node: Node, pub depth: usize, pub reason: String }
#[derive(Debug, Clone, Serialize)]
...
mod tests {
    ...
    fn sample() -> Graph {
        let input = Input {
            ...
            symbols: vec![
                CodeSymbol { key: 1, name: "a".into(), qualified_name: "a".into(), signature: "fn a()".into(), path: "src/a.rs".into(), start_line: 1, end_line: 1 },
                CodeSymbol { key: 2, name: "b".into(), qualified_name: "b".into(), signature: "fn b()".into(), path: "src/a.rs".into(), start_line: 2, end_line: 2 },
            ],
            edges: vec![CodeEdge { from: 1, to: 2, relation: "Calls".into() }],
            ..Input::default()
        };
        links::build(&input).unwrap()
    }
    #[test] fn impact_walks_reverse_dependencies() {
        let graph = sample(); let index = AnalysisIndex::new(&graph);
        let report = index.impact(&graph, &ImpactRequest { repo: "core".into(), files: vec!["src/a.rs".into()], max_depth: 2, max_nodes: 20 }).unwrap();
        assert_eq!(report.missing_files.len(), 0);
        assert_eq!(report.changed_nodes.len(), 3); // file + two symbols
    }
    #[test] fn dead_code_is_conservative_candidate_only() {
        let graph = sample(); let index = AnalysisIndex::new(&graph);
        let report = index.dead_code(&graph, &DeadCodeRequest { repo: Some("core".into()), limit: 10 }).unwrap();

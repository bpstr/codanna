...
pub fn scan(roots: &BTreeMap<String, PathBuf>) -> Result<Report> {
    ...
    for dep in &mut dependencies {
        if let Some(provider) = providers.get(&dep.package) {
            if provider != &dep.repo { dep.local_provider = Some(provider.clone()); local_links += 1; }
        }
    }
...
fn is_manifest(name: &str) -> bool { matches!(name, "package.json" | "composer.json" | "Cargo.toml" | "pyproject.toml" | "go.mod") }
...
fn parse_go(repo: &str, path: &str, text: &str, packages: &mut Vec<Package>, dependencies: &mut Vec<Dependency>) -> Result<()> {
    let mut module = None; let mut in_require = false;
...
mod tests {
    ...
    #[test] fn npm_local_link_is_detected() {
        ...
        let report = scan(&roots).unwrap(); assert_eq!(report.local_links, 1); assert_eq!(report.dependencies[0].local_provider.as_deref(), Some("b"));
    }
    #[test] fn go_require_block_is_parsed() {
        let root = tempfile::tempdir().unwrap();
        std::fs::write(root.path().join("go.mod"), "module example/a\nrequire (\n example/b v1.2.3\n)\n").unwrap();
        let report = scan(&BTreeMap::from([("a".into(), root.path().to_path_buf())])).unwrap(); assert_eq!(report.dependencies[0].package, "example/b");
    }
}

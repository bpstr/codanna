...
use crate::knowledge::{MAX_FILE_BYTES, Result, io, validate_repo};
use serde::Serialize;
...
pub fn scan(roots: &BTreeMap<String, PathBuf>) -> Result<Report> {
    if roots.is_empty() || roots.len() > 32 {
        return Err("manifest scan requires 1..32 repository roots".into());
    }
    ...
    for (repo, root) in roots {
            ...
            .build()
        {
            ...
            seen += 1;
            manifests += 1;
            if seen > 256 || manifests > 2048 {
                return Err("manifest scan limit exceeded".into());
            }
            let rel = path
                .strip_prefix(&root)?
                .to_str()
                .ok_or("non UTF-8 manifest path")?
                .replace('\\', "/");
    ...
    let providers: BTreeMap<String, String> = packages
        .iter()
        .filter_map(|p| p.name.as_ref().map(|name| (name.clone(), p.repo.clone())))
        .collect();
    ...
    dependencies.sort_by(|a, b| {
        (&a.repo, &a.manifest, &a.scope, &a.package).cmp(&(
            &b.repo,
            &b.manifest,
            &b.scope,
            &b.package,
    ...
    Ok(Report { packages, dependencies, local_links, scanned_manifests: manifests, limitations: vec![
        ...
        if let Some(items) = value.get(*section).and_then(JsonValue::as_object) {
            for (package, requirement) in items {
                dependencies.push(Dependency {
                    ...
                    requirement: requirement.as_str().unwrap_or("<structured>").into(),
                    scope: (*section).into(),
                    local_provider: None,
                });
            }
        ...
        for (package, requirement) in items {
            ...
            dependencies.push(Dependency {
                ...
                requirement: requirement
                    ...
                    .map(str::to_owned)
                    .unwrap_or_else(|| requirement.to_string()),
                scope: "poetry.dependencies".into(),
                local_provider: None,
            });
        }
    }
...
fn parse_go(
    ...
    for raw in text.lines() {
        ...
        if let Some(value) = line.strip_prefix("module ") {
            module = Some(value.trim().to_owned());
            continue;
        }
        if line == "require (" {
            in_require = true;
...
mod tests {
    use super::*;
    #[test]
    fn npm_local_link_is_detected() {
        let a = tempfile::tempdir().unwrap();
        let b = tempfile::tempdir().unwrap();
        std::fs::write(
            a.path().join("package.json"),
            r#"{"name":"@x/a","dependencies":{"@x/b":"workspace:*"}}"#,
        )
        .unwrap();
        std::fs::write(b.path().join("package.json"), r#"{"name":"@x/b"}"#).unwrap();
        let roots = BTreeMap::from([
            ("a".into(), a.path().to_path_buf()),
            ("b".into(), b.path().to_path_buf()),
        ]);

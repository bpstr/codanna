...
pub fn read_bounded(path: &Path, limit: u64) -> Result<Vec<u8>> {
    ...
    if bytes.len() as u64 > limit {
        return Err(format!("file exceeds byte limit: {}", path.display()).into());
    }
    Ok(bytes)
}
...
pub fn save(graph: &Graph, path: &Path) -> Result<()> {
    ...
    temporary.flush()?;
    if temporary.as_file().metadata()?.len() > MAX_GRAPH_BYTES {
        return Err("graph exceeds byte limit".into());
    }
...
pub fn source_path(root: &Path, relative: &str) -> Result<PathBuf> {
    ...
    if !path.starts_with(&root) {
        return Err("source resolves outside repository root".into());
    }
    Ok(path)
}
...
pub fn input(root: &Path, repo: &str, dump: Option<&Path>) -> Result<Input> {
    ...
    let dump = match dump {
        ...
        None => {
            ...
            let status = Command::new("codanna")
                .arg("dump")
                .current_dir(&root)
                .stdin(Stdio::null())
                .stdout(Stdio::from(generated.reopen()?))
                .status()?;
            if !status.success() {
                return Err("codanna dump failed; index code first or provide --dump".into());
            }
        ...
        .build()
    {
        ...
        if relative
            .split('/')
            .any(|p| matches!(p, ".git" | ".codanna" | "node_modules" | "target"))
        {
            continue;
        }
...
pub fn parse_dump(reader: impl BufRead, repo: &str) -> Result<Input> {
    ...
    for line in reader.lines() {
        ...
        if ended {
            return Err("data after dump summary".into());
        }
        ...
        match row["type"].as_str() {
            ...
            Some("result") if began => {
                if row["status"] != "success" {
                    return Err("unsuccessful dump row".into());
                }
                let data = &row["data"];
                match row["meta"]["entity_type"].as_str() {
                    Some("symbol") => {
                        ...
                        let path = string(data, "file_path")?;
                        validate_path(&path)?;
                        let start = data["range"]["start_line"]
                            .as_u64()
                    ...
                    Some("relationship") => input.edges.push(CodeEdge {
                        ...
                        to: data["to"]["id"].as_u64().ok_or("missing edge target")?,
            ...
            Some("summary") if began => {
                if row["status"] != "success" {
                    return Err("unsuccessful dump summary".into());
                }
                expected = Some((
                    row["data"]["symbols"]
                        .as_u64()
                        .ok_or("missing symbol count")?,
                    row["data"]["relationships"]
                        .as_u64()
                        .ok_or("missing relationship count")?,
                ));
                ended = true;
            }
    ...
    if expected != Some((input.symbols.len() as u64, input.edges.len() as u64)) {
        return Err("missing summary or dump count mismatch".into());
    }
    Ok(input)
}
...
{\"type\":\"summary\",\"status\":\"success\",\"data\":{\"symbols\":1,\"relationships\":0}}\n";
    ...
    fn replacement_roundtrip_and_size_limit() {
        ...
        save(&Graph::default(), &path).unwrap();
        load(&path).unwrap();

...
fn communities(graph: &Graph) -> Vec<Community> {
    let adj = adjacency(graph);
    let mut labels: BTreeMap<String, String> = graph
        .nodes
        .keys()
    ...
    for _ in 0..20 {
        ...
        for id in graph.nodes.keys() {
            ...
            if best < labels[id] && best != labels[id] {
    ...
    for (index, (_label, mut members)) in groups.into_iter().enumerate() {
        ...
        for id in &members {
            repos.insert(graph.nodes[id].source.repo.clone());
        }
        for edge in graph.edges.iter().filter(|e| usable(e)) {
            let a = set.contains(&edge.from);
            let b = set.contains(&edge.to);
            if a && b {
                internal += 1;
            } else if a || b {
                boundary += 1;
        ...
        result.push(Community {
            id: index,
            label: graph.nodes[hub].label.clone(),
            members,
...
pub fn analyze(graph: &Graph, hub_limit: usize) -> Result<ArchitectureReport> {
    graph.validate()?;
    if !(1..=100).contains(&hub_limit) {
        return Err("hub limit must be 1..100".into());
    }
    let adj = adjacency(graph);
    let mut hubs: Vec<_> = graph
        ...
        .map(|node| Hub {
            ...
            degree: adj.get(&node.id).map_or(0, |v| v.len()),
            repos: vec![node.source.repo.clone()],
        })
        .collect();
    hubs.sort_by(|a, b| b.degree.cmp(&a.degree).then_with(|| a.id.cmp(&b.id)));
    hubs.truncate(hub_limit);
    let cross_repository_edges = graph
        .edges
        .iter()
        .filter(|e| usable(e) && graph.nodes[&e.from].source.repo != graph.nodes[&e.to].source.repo)
        .count();
...
mod tests {
    use super::*;
    #[test]
    fn deterministic_and_candidate_free() {
        let input = Input {
            repo: "r".into(),
            files: BTreeMap::from([("a.rs".into(), "fn a(){}\nfn b(){}".into())]),
            symbols: vec![
                CodeSymbol {
                    key: 1,
                    name: "a".into(),
        ...
        let a = analyze(&graph, 10).unwrap();
        let b = analyze(&graph, 10).unwrap();
        assert_eq!(
    ...
    #[test]
    fn candidate_does_not_join_components() {
        let mut graph = Graph::default();
        for (i, label) in ["a", "b"].into_iter().enumerate() {
            let path = format!("{label}.md");
            ...
            let id = identity("r", "Document", &path, "");
            graph.nodes.insert(
                id.clone(),
                Node {
                    id,
        ...
        graph.edges.push(Edge {
            ...
            evidence: graph.nodes[&ids[0]].source.clone(),
            method: "test".into(),
        });
        graph.validate().unwrap();
        assert_eq!(analyze(&graph, 2).unwrap().connected_components, 2);
    }
}

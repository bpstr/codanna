...
fn corpus(count: usize) -> Vec<String> {
    (0..count).map(|index| format!("synthetic input {index}")).collect()
}
...
fn populated(inputs: &[String], dimension: usize) -> EmbeddingCache {
    ...
    for input in inputs { assert!(cache.insert(input, Arc::clone(&vector))); }
    cache
}
...
fn scan(mut cache: EmbeddingCache, inputs: &[String], admit_misses: bool) -> usize {
    ...
    for batch in inputs.chunks(64) {
        let missing: Vec<_> = batch.iter().filter(|input| cache.get(input).is_none()).collect();
        hits += batch.len() - missing.len();
        if admit_misses {
            for input in missing { assert!(cache.insert(input, Arc::clone(&vector))); }
        }
    }
    hits
}
...
fn snapshot_matches_can_be_lost_to_sequential_admission() {
    ...
    let snapshot_hits = inputs.iter().filter(|input| cache.get(input).is_some()).count();
    ...
    println!("inputs=4500 snapshot_hits={snapshot_hits} fifo_batch64_hits={fifo_hits} no_admission_experiment_hits={pinned_experiment_hits} reverse_scan_hits={reverse_hits}");
    assert_eq!(snapshot_hits, 4096);
    assert_eq!(fifo_hits, 0, "sequential misses evict not-yet-consumed resident inputs");
...
fn cache_byte_budget_can_bind_before_the_entry_count_limit() {
    ...
    let resident = inputs.iter().filter(|input| cache.get(input).is_some()).count();
...
fn persisted_cache_keeps_the_same_pressure_boundary() {
    ...
    assert_eq!(inputs.iter().filter(|input| cache.get(input).is_some()).count(), 4096);
    assert_eq!(scan(cache, &inputs, true), 0);
    assert_eq!(std::fs::read(&path).unwrap(), before, "simulation must not rewrite the saved cache");
}

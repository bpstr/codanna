...
impl ServerHandler for Reader {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            .with_server_info(Implementation::new("codanna-workspace-reader", env!("CARGO_PKG_VERSION")))
    }
    ...
    async fn call_tool(&self, request: CallToolRequestParams, context: RequestContext<RoleServer>) -> Result<CallToolResponse, ErrorData> {
        if matches!(request.name.as_ref(), "semantic_search_docs" | "semantic_search_with_context") && self.settings.semantic_search.enabled {
            let facade = self.code.facade.clone();
            let path = self.settings.index_path.join("semantic");
            let outcome = self.semantic.get_or_init(|| async move {
                crate::runtime::mutate(&facade, move |index| {
                    ...
                    if index.has_semantic_search() { Ok(true) } else { index.load_semantic_search(&path) }
        ...
        if matches!(request.name.as_ref(), "search_context" | "search_documents") {
            ...
            let documents = self.documents.get_or_init(|| async move {
                crate::runtime::blocking(move || crate::documents::load_from_settings(&settings)).await.unwrap_or(None)
            }).await;
            if let Some(store) = documents { server = server.with_document_store_arc(store.clone()); }
        }
...
pub(crate) async fn run(root: &Path) -> Result<i32, IndexError> {
    let root = root.canonicalize().map_err(|source| IndexError::FileRead { path: root.to_path_buf(), source })?;
    if std::env::current_dir().map_err(|e| IndexError::General(e.to_string()))?.canonicalize().map_err(|e| IndexError::General(e.to_string()))? != root {
    ...
    settings.indexed_paths_cache = settings.indexing.indexed_paths.iter().map(|path| root.join(path).canonicalize()).collect::<Result<Vec<_>, _>>().map_err(|e| IndexError::General(e.to_string()))?;
    ...
    let facade = crate::runtime::blocking(move || IndexPersistence::new(owned.index_path.clone()).load_facade_lite(owned)).await??;
    let reader = Reader { code: CodeIntelligenceServer::new(facade), settings, semantic: Arc::new(OnceCell::new()), documents: Arc::new(OnceCell::new()) };
    let running = reader.serve(rmcp::transport::stdio()).await.map_err(|e| IndexError::General(e.to_string()))?;
    running.waiting().await.map_err(|e| IndexError::General(e.to_string()))?;
    Ok(0)
}

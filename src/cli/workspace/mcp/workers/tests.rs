...
impl ServerHandler for ControlledReader {
    ...
    async fn call_tool(&self, request: CallToolRequestParams, context: RequestContext<RoleServer>) -> Result<CallToolResponse, ErrorData> {
        if request.name == "invalid" {
            return Err(ErrorData::invalid_params("fixture invalid parameters", None));
        }
...
async fn hardening_workspace_pool_overlaps_queries_and_cancels_only_the_request() {
    ...
    let backend = ControlledReader { events, release: release.clone() };
    ...
    let workspace = Workspace {
        id: WorkspaceId::new(), name: "fixture".into(), root: temporary.path().to_path_buf(),
        config_path: temporary.path().join("settings.toml"),
    };
    let stamp = Stamp { length: 0, modified: SystemTime::UNIX_EPOCH, identity: (0, 0) };
    let reader = Arc::new(Reader {
        peer: client.peer().clone(), queries: Semaphore::new(4), close: CancellationToken::new(),
        snapshot: Snapshot { root: workspace.root.clone(), index: workspace.root.clone(), config: stamp.clone(), metadata: stamp.clone(), tantivy: stamp },
    });
    let (_sender, receiver) = watch::channel(Event::Ready(reader));
    pool.slots.lock().insert(workspace.id.as_str().into(), Arc::new(Mutex::new(Slot {
        event: Some(receiver), refresh_at: Instant::now() + Duration::from_secs(60), last_used: Instant::now(),
    })));
    let cancel_a = CancellationToken::new();
    let a = {
        let pool = pool.clone(); let workspace = workspace.clone(); let token = cancel_a.clone();
        tokio::spawn(async move { pool.call(&workspace, CallToolRequestParams::new("blocked-a"), token).await })
    };
    let b = {
        let pool = pool.clone(); let workspace = workspace.clone();
        tokio::spawn(async move { pool.call(&workspace, CallToolRequestParams::new("blocked-b"), CancellationToken::new()).await })
    };
    let mut entered = Vec::new();
    for _ in 0..2 {
        entered.push(tokio::time::timeout(Duration::from_secs(2), observed.recv()).await.unwrap().unwrap());
    }
    entered.sort();
    assert_eq!(entered, ["blocked-a", "blocked-b"]);
    cancel_a.cancel();
    assert!(tokio::time::timeout(Duration::from_secs(2), a).await.unwrap().unwrap().is_err());
    assert_eq!(tokio::time::timeout(Duration::from_secs(2), observed.recv()).await.unwrap().unwrap(), "cancelled:blocked-a");
    release.add_permits(1);
    assert!(tokio::time::timeout(Duration::from_secs(2), b).await.unwrap().unwrap().is_ok());
    assert!(!client.peer().is_transport_closed());
    let error = pool.call(&workspace, CallToolRequestParams::new("invalid"), CancellationToken::new()).await.unwrap_err();
    assert_eq!(error.code, ErrorCode::INVALID_PARAMS);
    assert_eq!(error.message, "fixture invalid parameters");
    assert!(pool.call(&workspace, CallToolRequestParams::new("valid"), CancellationToken::new()).await.is_ok());

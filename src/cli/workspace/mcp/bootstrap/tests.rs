...
fn hardening_workspace_bootstrap_requires_indexed_files_not_just_directory_contents() {
    assert!(matches!(validate_generation(&generation(0)).unwrap(), Status::Empty));
    ...
    assert!(matches!(validate_generation(&generation(1)).unwrap(), Status::Ready));
    assert!(matches!(validate_generation(&generation(MAX_FILES)).unwrap(), Status::Ready));
...
fn prepared_build(temp: &tempfile::TempDir, files: u32) -> Build {
    ...
    let lock = OpenOptions::new().read(true).write(true).create(true).truncate(false)
        .open(state.join("bootstrap.lock")).unwrap();
    ...
    generation(files).save(&staging.path().join("index")).unwrap();
    Build {
        _lock: lock,
...
fn hardening_workspace_bootstrap_empty_generation_is_not_published() {
    ...
    assert!(matches!(publish(build, &CancellationToken::new()).unwrap(), Status::Empty));
...
fn hardening_workspace_bootstrap_changed_configuration_cannot_publish_old_scope() {
    ...
    let destination = build.destination.clone();
    fs::write(&build.source_config, "[indexing]\nindexed_paths = [\"different\"]\n").unwrap();
...
fn hardening_workspace_bootstrap_overflow_and_cancellation_preserve_existing_storage() {
    for cancelled in [false, true] {
        ...
        assert_eq!(fs::read(destination.join("sentinel")).unwrap(), b"existing data");
    }
}

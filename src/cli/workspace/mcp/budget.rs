...
impl Budget {
    pub(super) fn new() -> Self { Self { requests: Arc::new(Semaphore::new(16)), blocking: Arc::new(Semaphore::new(4)) } }
    pub(super) fn enter(&self) -> Result<OwnedSemaphorePermit, ErrorData> {
        self.requests.clone().try_acquire_owned().map_err(|_| super::internal("Workspace request queue is full"))
    }
    pub(super) async fn run<T, F>(&self, ct: &CancellationToken, work: F) -> Result<T, ErrorData>
    where T: Send + 'static, F: FnOnce(CancellationToken) -> Result<T, ErrorData> + Send + 'static {
        ...
        let mut task = tokio::task::spawn_blocking(move || {
            let _permit = permit;
            if work_ct.is_cancelled() { return Err(super::internal("Workspace operation cancelled")); }
            work(work_ct)
        });
...
mod tests {
    ...
    async fn hardening_workspace_blocking_permit_outlives_cancelled_waiter() {
        ...
        let clone = budget.clone(); let token = ct.clone();
        let caller = tokio::spawn(async move { clone.run(&token, move |_| { let _ = started.send(()); wait.recv_timeout(Duration::from_secs(5)).unwrap(); Ok(()) }).await });
        started_rx.await.unwrap(); ct.cancel();
        assert!(caller.await.unwrap().is_err());
        assert_eq!(budget.blocking.available_permits(), 3);
        release.send(()).unwrap();
        tokio::time::timeout(Duration::from_secs(2), async { while budget.blocking.available_permits() != 4 { tokio::task::yield_now().await; } }).await.unwrap();
    }
}

...
impl State {
    fn new(status: &'static str, ready: bool) -> Self { Self { status, ready, detail: None } }
}
    ...
    pub(super) fn start(facade: Facade, settings: Arc<Settings>) -> Arc<Self> {
        ...
        let task = tokio::spawn(async move {
            if let Err(error) = run(facade, settings, state_task.clone(), token).await {
                *state_task.lock() = State { status: "unavailable", ready: false, detail: Some(error.to_string()) };
            }
        });
        Arc::new(Self { state, stop, task: Mutex::new(Some(task)) })
    }
    pub(super) fn state(&self) -> State { self.state.lock().clone() }
...
impl Drop for LiveWorkspace {
    fn drop(&mut self) { self.stop.cancel(); }
}
...
fn context_stamp(root: &Path) -> Result<ContextStamp, IndexError> {
    fn stamp(path: &Path) -> Result<Option<(u64, SystemTime)>, IndexError> {
        match std::fs::symlink_metadata(path) {
            ...
            Ok(_) => Err(IndexError::General("Workspace settings/ignore file must be regular".into())),
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(None),
            Err(error) => Err(error.into()),
        }
    }
    Ok(ContextStamp {
        config: stamp(&crate::cli::automatic::config_path(root))?.ok_or_else(|| IndexError::General("Workspace configuration disappeared".into()))?,
        ignore: stamp(&root.join(".codannaignore"))?,
    })
}

pub(super) fn eligible(settings: &Settings) -> bool {
    settings.file_watch.enabled && !settings.semantic_search.enabled
        && !settings.index_path.join("semantic/metadata.json").exists()
}

async fn run(facade: Facade, settings: Arc<Settings>, state: Arc<Mutex<State>>, stop: CancellationToken) -> Result<(), IndexError> {
    let owned = settings.clone();
    let automatic = crate::runtime::blocking(move || eligible(&owned)).await?;
    if !automatic {
        *state.lock() = State::new(if settings.file_watch.enabled { "semantic-manual" } else { "disabled" }, true);
        return Ok(());
    }
    let root = settings.workspace_root.clone().ok_or_else(|| IndexError::General("Missing live workspace root".into()))?;
    ...
    loop {
        ...
        let lease = crate::runtime::blocking(move || {
            if context_stamp(&checked)? != expected {
                return Err(IndexError::General("Workspace configuration changed; the router must reload this reader".into()));
            }
        ...
        *state.lock() = State::new("refreshing", false);
        ...
        crate::runtime::mutate(&facade, move |index| {
            ...
            if !eligible(&owned) {
                return Err(IndexError::General("Semantic configuration changed; reload the workspace reader".into()));
            }
            ...
            if metadata.emission_version != Some(crate::storage::EMISSION_SEMANTICS_VERSION) {
                return Err(IndexError::General("Index format changed; an explicit rebuild is required".into()));
            }
            let mut refreshed = IndexPersistence::new(owned.index_path.clone()).load_facade_lite(owned)?;
        ...
        let mut watcher = UnifiedWatcher::builder()
            ...
            .build().map_err(|error| IndexError::General(error.to_string()))?;
        watcher.prepare().await.map_err(|error| IndexError::General(error.to_string()))?;
        catch_up(&facade, &settings, &root, &initial, &stop, lease.clone()).await?;
        *state.lock() = State::new("watching", true);
        ...
        watch.await.map_err(|error| IndexError::General(error.to_string()))?;
        drop(lease);
        return outcome;
    }
}

async fn catch_up(facade: &Facade, settings: &Arc<Settings>, root: &Path, expected: &ContextStamp, stop: &CancellationToken, lease: Arc<CodeWriteLease>) -> Result<(), IndexError> {
    ...
    crate::runtime::mutate(facade, move |index| {
        ...
        if context_stamp(&root)? != expected {
            return Err(IndexError::General("Workspace configuration changed during catch-up".into()));
        }
        let roots: Vec<PathBuf> = if settings.indexed_paths_cache.is_empty() { vec![root.clone()] } else { settings.indexed_paths_cache.clone() };
        for source in &roots {
            if source.canonicalize()? != *source || !source.starts_with(&root) {
                return Err(IndexError::General("Live source escaped the workspace".into()));
            }
        }

//! Workspace administration and fixed-context process launch adapters.
...
pub enum WorkspaceAction {
    /// Serve independent local workspaces with automatic first-use code indexing
    Serve,
    /// Internal strict, lazy-model reader; launched by the workspace router
    #[command(hide = true)]
    Reader { root: PathBuf },
...
pub fn run(action: &WorkspaceAction) -> Result<i32, IndexError> {
    // The existing binary calls this dispatcher from its multi-thread runtime.
    // Never take stdout's synchronous lock across an MCP transport lifetime.
    match action {

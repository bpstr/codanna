...
impl Default for Slot {
    fn default() -> Self {
        Self { client: None, fingerprint: Vec::new(), last_used: Instant::now(), retry_after: None }
    }
}
...
impl WorkerPool {
    pub(super) fn new(registry: WorkspaceRegistry, executable: PathBuf) -> Self {
        Self { registry, executable, slots: Mutex::new(HashMap::new()), admission: Semaphore::new(MAX_REQUESTS) }
    }
    ...
    async fn slot(&self, id: &str) -> Result<Arc<Mutex<Slot>>, ErrorData> {
        ...
        slots.retain(|_, slot| {
            Arc::strong_count(slot) != 1
                || slot.try_lock().map_or(true, |slot| slot.last_used.elapsed() < IDLE_TIMEOUT)
        });
        if let Some(slot) = slots.get(id) {
            return Ok(Arc::clone(slot));
        }
        if slots.len() >= MAX_WORKERS {
            let oldest = slots.iter().filter(|(_, slot)| Arc::strong_count(slot) == 1)
                .filter_map(|(id, slot)| slot.try_lock().ok().map(|slot| (id.clone(), slot.last_used)))
                .min_by_key(|(_, used)| *used).map(|(id, _)| id);
            match oldest {
                Some(id) => { slots.remove(&id); }
                None => return Err(super::internal("All workspace workers are busy; retry the query")),
            }
        }
    ...
    pub(super) async fn is_loaded(&self, id: &str) -> bool {
        ...
        slots.get(id).is_some_and(|slot| slot.try_lock().is_ok_and(|slot| slot.client.is_some()))
    }
    ...
    pub(super) async fn call(
        ...
        let _admission = self.admission.try_acquire()
            .map_err(|_| super::internal("Workspace query queue is full; retry later"))?;
        let registry = self.registry.clone();
        let selected = workspace.clone();
        let executable = self.executable.clone();
        let (command, fingerprint) = tokio::task::spawn_blocking(move || {
            worker_command(&registry, &selected, &executable)
        ...
        if slot.retry_after.is_some_and(|until| until > Instant::now()) {
            return Err(super::internal(format!("Workspace '{}' reader is cooling down after failure", workspace.name)));
        }
        ...
        if slot.client.is_none() {
            ...
            match started {
                ...
                result => {
                    ...
                    return Err(super::internal(format!("Workspace '{}' could not start: {detail}. Run codanna index in its root if rebuilding is required.", workspace.name)));
                }
            }
        }
        let result = {
    ...
    pub(super) async fn shutdown(&self) {
        let slots: Vec<_> = self.slots.lock().await.drain().map(|(_, slot)| slot).collect();
        for slot in slots {
...
fn worker_command(
    ...
    {
        return Err(super::internal("Workspace readers must use the running Codanna executable"));
    }
    let current = registry.get(workspace.id.as_str()).map_err(super::internal)?;
    if current.root != workspace.root {
        return Err(super::internal("Workspace moved during selection; retry with its stable ID"));
    }
    let settings = read_settings(&current.root).map_err(super::internal)?;
    if settings.server.mode == "http" {
        return Err(super::internal("Local workspace readers require stdio settings; use the explicitly configured network server instead"));
    }
    let index = confined_index_path(&current.root, &settings).map_err(super::internal)?;
    crate::storage::IndexMetadata::load(&index).map_err(|error| super::internal(format!(
        "Workspace '{}' index is unavailable: {error}. Run codanna index in its root.", current.name,
    )))?;
    let args = vec![OsString::from("--workspace"), current.id.as_str().into(), OsString::from("serve")];
    let plan = WorkspaceLaunch::prepare(registry, current.id.as_str(), &args).map_err(super::internal)?;
    let mut fingerprint = Sha256::new();
    fingerprint.update(current.root.as_os_str().as_encoded_bytes());
    for path in [current.config_path.clone(), index.join("index.meta"), index.join("tantivy/meta.json")] {
        let mut bytes = Vec::new();
        File::open(&path).map_err(super::internal)?.take(1024 * 1024 + 1)
            .read_to_end(&mut bytes).map_err(super::internal)?;
        if bytes.len() > 1024 * 1024 {
            return Err(super::internal("Workspace metadata exceeds the 1 MiB routing limit"));
        }

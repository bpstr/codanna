...
impl Fixture {
    fn new() -> Self {
        ...
        for (id, name, doc, signature, path) in [
            (1, "useAccountPresentation", "Calendar settings select the first weekday for an account.", "export function useAccountPresentation()", "src/calendar/presentation.ts"),
            (2, "get_current_account", "Read effective account preferences.", "fn get_current_account() -> Account", "src/account.rs"),
            (3, "ArchiveService", "Store a completed project.", "class ArchiveService", "src/archive.ts"),
            (4, "render", "Render a typed value.", "fn render<T>(value: T) -> View", "src/render.rs"),
        ] {
            add(&index, id, name, doc, signature, path);
        }
        for id in 10..58 {
            add(&index, id, "settings", "Generic application settings.", "const settings = {}", &format!("src/generic/{id}.ts"));
        }
...
fn add(index: &DocumentIndex, id: u32, name: &str, doc: &str, signature: &str, path: &str) {
    let mut symbol = Symbol::new(
        ...
        FileId::new(id).unwrap(), Range::new(0, 0, 0, 50),
    );
...
fn main_clause(index: &DocumentIndex, text: &str) -> Box<dyn Query> {
    parsed(index, vec![index.schema.name_text, index.schema.doc_comment,
        index.schema.signature, index.schema.context], text)
}

fn fuzzy(index: &DocumentIndex, field: Field, text: &str) -> Box<dyn Query> {
    let _ = index;
    Box::new(FuzzyTermQuery::new(Term::from_field_text(field, text), 1, true))
}

fn combined(index: &DocumentIndex, text: &str) -> BooleanQuery {
    BooleanQuery::new(vec![
        (Occur::Must, Box::new(BooleanQuery::new(vec![
        ...
        (Occur::Must, Box::new(TermQuery::new(
            Term::from_field_text(index.schema.doc_type, "symbol"), IndexRecordOption::Basic,
        ))),
    ])
}

fn rows(index: &DocumentIndex, searcher: &Searcher, query: &dyn Query) -> Vec<(u32, f32, DocAddress)> {
    searcher.search(query, &TopDocs::with_limit(200).order_by_score()).unwrap()
        .into_iter().map(|(score, address)| {
            let doc: TantivyDocument = searcher.doc(address).unwrap();
            let id = doc.get_first(index.schema.symbol_id).and_then(|value| value.as_u64()).unwrap();
...
fn ranking_diagnostics_measure_fields_fuzzy_clauses_and_score_trees() {
    ...
    let owner_rank = raw.iter().position(|(id, _, _)| *id == 1).map(|rank| rank + 1);
    assert!(owner_rank.is_some(), "owner must exist in the lexical candidate pool");

    let components: Vec<(&str, Box<dyn Query>)> = vec![
        ("all_analyzed_fields", main_clause(index, query)),
        ("analyzed_name_only", parsed(index, vec![index.schema.name_text], query)),
        ("documentation_only", parsed(index, vec![index.schema.doc_comment], query)),
        ("signature_only", parsed(index, vec![index.schema.signature], query)),
        ("whole_query_fuzzy_ngram", fuzzy(index, index.schema.name_text, query)),
        ("whole_query_fuzzy_name", fuzzy(index, index.schema.name, query)),
    ];
    ...
    assert_eq!(explained.len(), 2, "both the owner and a generic decoy need explanations");
    let selected = index.search(query, 5, None, None, None).unwrap();
    assert!(selected.iter().any(|result| result.symbol_id.value() == 1));
    assert_eq!(fuzzy(index, index.schema.name_text, query).count(&searcher).unwrap(), 0);
    assert_eq!(fuzzy(index, index.schema.name, query).count(&searcher).unwrap(), 0);
    println!("ranking_clause_evidence={}", json!({
...
fn ranking_diagnostics_preserve_raw_scores_across_query_families() {
    ...
    for (family, query) in [
        ...
        for result in &results {
            let raw_score = scores.get(&result.symbol_id.value()).expect("runtime returned a non-candidate");
            assert!((result.score - raw_score).abs() < 0.0001, "score drift in {family}");
        }
        println!("ranking_query_family={}", json!({
    ...
    assert_eq!(index.search("ArchivService", 1, None, None, None).unwrap()[0].name, "ArchiveService");
    assert_eq!(index.search("get_current_account", 1, None, None, None).unwrap()[0].name, "get_current_account");
    assert!(index.search("qzvnoexist75391", 5, None, None, None).unwrap().is_empty());
}

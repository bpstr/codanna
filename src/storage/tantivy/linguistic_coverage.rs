...
fn identifier_words(text: &str) -> String {
    ...
    while let Some(character) = chars.next() {
        if character.is_uppercase() && previous.is_some_and(|previous| {
            previous.is_lowercase()
                || (previous.is_uppercase() && chars.peek().is_some_and(|next| next.is_lowercase()))
        }) {
            output.push(' ');
        }
...
pub(super) fn query_terms(query: &str) -> Option<Vec<String>> {
    ...
    if query.chars().any(|character| matches!(character,
        ':' | '"' | '(' | ')' | '[' | ']' | '{' | '}' | '~' | '*' | '?' | '\\' | '/'
        | '^' | '+' | '-' | '<' | '>' | '!' | '|' | '&'
    )) || query.split_whitespace().any(|word| matches!(word, "AND" | "OR" | "NOT" | "IN")) {
        return None;
    }
    ...
    let terms: Vec<_> = normalized_words(query).into_iter().take(MAX_QUERY_TERMS).collect();
...
pub(super) fn coverage(result: &SearchResult, terms: &[String]) -> usize {
    ...
    for field in [
        Some(result.name.as_str()), result.doc_comment.as_deref(),
        result.signature.as_deref(), result.context.as_deref(),
        Some(result.module_path.as_str()), Some(result.file_path.as_str()),
    ].into_iter().flatten() {
        evidence.push(' ');
...
mod tests {
    ...
    fn row(name: &str, doc: &str, path: &str) -> SearchResult {
        SearchResult {
            symbol_id: SymbolId::new(1).unwrap(), name: name.into(), kind: SymbolKind::Function,
            file_path: path.into(), line: 1, column: 0, doc_comment: Some(doc.into()),
            signature: None, module_path: String::new(), language_id: Some("rust".into()),
            score: 1.0, highlights: Vec::new(), context: None,
        }
    }
    ...
    fn linguistic_coverage_matches_morphology_and_identifier_parts_not_substrings() {
        ...
        assert_eq!(coverage(&row("vectorNamespace", "", "src/api.rs"), &terms), 2);
        let terms = query_terms("raw response").unwrap();
        assert_eq!(coverage(&row("redraw", "response", "src/api.rs"), &terms), 1);
        assert_eq!(coverage(&row("read_file", "read bytes", "src/recall.rs"), &query_terms("read recall").unwrap()), 2);
        assert!(normalized_words("HTTPServer read_file árvíz").is_superset(&BTreeSet::from([
            "http".into(), "server".into(), "read".into(), "file".into(), "árvíz".into(),
        ])));
    }

    #[test]
    fn linguistic_coverage_preserves_identifier_and_explicit_syntax_paths() {
        for query in ["get_current_account", "ArchiveService", "ui", "calendar AND settings",
            "\"calendar settings\"", "doc_comment:calendar", "calendar -generic",
            "render<T> settings", "x IN [a b]", "alpha || beta", "alpha +beta"] {
            assert!(query_terms(query).is_none(), "must bypass discovery: {query}");
        }
    ...
    fn linguistic_coverage_deduplicates_inflections_and_bounds_query_terms() {
        assert_eq!(query_terms("vectors vector namespaces namespace"), query_terms("vector namespace"));
        let long = (0..100).map(|i| format!("token{i}")).collect::<Vec<_>>().join(" ");
        assert_eq!(query_terms(&long).unwrap().len(), MAX_QUERY_TERMS);
        assert_eq!(coverage(&row("vector", "vector vector vectors", "vector.rs"), &query_terms("vector namespace").unwrap()), 1);
    }
}

...
4. Clarify validation, rendering, routing diagnostics, and historical lifecycle disposition (T09–T11); retain the resolved browser-scroll error attribution and separate caller-hardening follow-up (T14).
...
- [ ] P2, MCP; C07–C09; depends on T01. Keep all three context limits at 1–10 unless a separately measured budget change is accepted. Add field-specific errors and request-schema boundary fixtures for 0, 1, 10, 11, wrong type, unknown field, and omitted defaults. The current schema already declares the bounds. S7 confirms the original caller sent conversation_limit: 0; include that exact case. Consider an explicit skip-recall contract only as a separate schema/budget decision, rather than silently treating invalid zero as supported.
...
### T14 — Prevent malformed browser-scroll calls (external follow-up)

Attribution is complete from S7: two cua_repl.js scroll actions failed with malformed target arguments; the corrected positional-target call succeeded. Ten inspected Codanna find_symbol events have isError false. No Codanna implementation defect is established by these accessibility errors. This task records optional browser-caller hardening, not a search/router patch.

- [ ] P2, browser-action caller/integration owner; U01. Use the active documented action signature and a target from a current observed UI snapshot. Preserve the exact failed forms and successful historical correction in deterministic, mocked contract fixtures; do not reuse historical element 162 as a live target.
- [ ] Completion: missing/malformed targets fail clearly; documented observed-target calls pass; recovery checks the signature and observation before retrying instead of guessing object keys. No arbitrary live clicks/scrolls are needed for automated validation.
- [ ] Preserve the distinction between Codanna validation errors and browser-action errors in future reports, using the raw server/tool/result envelope and unique call IDs. Hand off any actual caller fix outside Codanna; do not alter ranking, workspace selection or accessibility permissions to suppress this message. No external issue submission or implementation is part of this packet.

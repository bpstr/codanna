...
use codanna::symbol_representation::{
    CodeEmbeddingPolicy, MAX_FILE_SOURCE_BYTES, MAX_SYMBOL_SOURCE_BYTES,
};
...
fn repository_body_inputs_capture_real_owners_without_embedding_them() {
    ...
    for &(relative, source) in SOURCES {
        ...
        let retained: usize = parsed
            .raw_symbols
            .iter()
            .filter_map(|symbol| symbol.embedding_source.as_ref())
            .map(|input| {
                input.header.len()
                    + input
                        .fragments
        ...
        for task in oracle["tasks"]
            ...
            .unwrap()
            .iter()
            .filter(|task| task["path"] == relative)
        {
    ...
    println!(
        "repository_body_summary={}",
        json!({"owners":owners_seen, "legacy_eligible":documented,
...
fn repository_body_inputs_keep_legacy_policy_opt_in() {
    ...
    assert!(
        parsed
            .raw_symbols
            .iter()
            .all(|symbol| symbol.embedding_source.is_none())
    );
}

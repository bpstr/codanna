...
fn fixture(policy: CodeEmbeddingPolicy) -> (tempfile::TempDir, IndexFacade) {
    ...
    std::fs::write(root.join("src/lib.rs"), "/// Conversation timeout dispatch.\npub fn dispatch_ticket() {}\n").unwrap();
    ...
    let mut metadata: serde_json::Value = serde_json::from_slice(&std::fs::read(&manifest).unwrap()).unwrap();
    metadata["embedding_identity"] = json!(json!({
...
fn assert_invalidated(facade: &IndexFacade, rejected_save: &Path) {
    assert!(facade.is_semantic_incompatible(), "failed reload left the previous vectors usable");
    let error = facade.semantic_search_docs("conversation timeout", 1).unwrap_err();
    assert!(error.to_string().contains("incompatible"), "{error}");
    assert!(facade.save_semantic_search(rejected_save).is_err());
    assert!(!rejected_save.exists(), "invalid state must not publish a checkpoint");
    let lexical = facade.search("conversation timeout", 1, None, None, None).unwrap();
    assert_eq!(lexical[0].name, "dispatch_ticket");
}

#[test]
fn malformed_reload_invalidates_previous_vectors_and_valid_reload_recovers() {
    for policy in [CodeEmbeddingPolicy::DocComment, CodeEmbeddingPolicy::SymbolBodyV1] {

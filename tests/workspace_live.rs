...
async fn wait_for(client: &Client, query: &str, expected: &str, forbidden: &str) -> String {
    ...
    .await
    .expect("native watcher must converge without explicit indexing")
}
async fn role(client: &Client) -> String {
    let result = call(client, "get_index_info", json!({})).await;
    result.structured_content.unwrap()["result"]["freshness"]["status"]
        .as_str()
        .unwrap_or("starting")
        .to_owned()
}
...
async fn hardening_workspace_live_edits_creates_deletes_and_restart_stay_isolated() {
    ...
    assert_eq!(
        fs::read(b.join(".codanna/index/index.meta")).unwrap(),
        original_b
    );
...
async fn hardening_workspace_live_elects_one_writer_and_takes_over_after_disconnect() {
    ...
    source(&root, "INITIAL");
    let a = connect(&home, &root).await;
    let b = connect(&home, &root).await;
    ...
    assert!(
        matches!(
            (a_role.as_str(), b_role.as_str()),
            ("watching", "following") | ("following", "watching")
        ),
        "{a_role}/{b_role}"
    );
    ...
    assert_eq!(
        fs::read(root.join(".codanna/index/index.meta")).unwrap(),
        metadata
    );
    let follower = if a_role == "watching" {
    ...
    tokio::time::timeout(Duration::from_secs(30), async {
        while role(&follower).await != "watching" {
            tokio::time::sleep(Duration::from_millis(100)).await;
        }
    })
    .await

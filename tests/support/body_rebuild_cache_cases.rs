...
const BODY_SOURCE: &str = "/// alpha owner.\npub fn alpha_owner() -> u8 { helper() }\npub fn helper() -> u8 { 1 }\n";
...
fn tree(root: &Path) -> BTreeMap<PathBuf, (Vec<u8>, Option<std::time::SystemTime>)> {
    ...
    while let Some(path) = pending.pop() {
        ...
        if metadata.is_dir() {
            ...
            result.insert(path.clone(), (std::fs::read(path).unwrap(), metadata.modified().ok()));
        }
    }
    result
}
...
fn plan(workspace: &Workspace, endpoint: &Endpoint) -> Value {
    ...
    let output = command.current_dir(workspace.root())
        .args(["--config", ".codanna/settings.toml"]).output().unwrap();
    assert!(output.status.success(), "{}", String::from_utf8_lossy(&output.stderr));
    assert_eq!(tree(workspace.root()), before, "planner changed the workspace");
    assert_eq!(endpoint.inputs.lock().unwrap().len(), request_count, "planner contacted endpoint");
...
fn sent(endpoint: &Endpoint, expected: usize) -> Vec<String> {
    ...
    let probes = inputs.iter().filter(|input| input.as_str() == "probe").count();
    let documents: Vec<_> = inputs.into_iter().filter(|input| input != "probe").collect();
    println!("body_reuse: source_inputs={} probes={probes} expected={expected}", documents.len());
    assert_eq!(probes, 1, "each rebuild still initializes its remote backend");
...
fn current_ids(workspace: &Workspace, names: &[&str]) -> HashSet<SymbolId> {
    ...
    let facade = IndexPersistence::new(index_path.clone()).load_facade_lite(Arc::new(settings)).unwrap();
    let ids: HashSet<_> = names.iter().map(|name| {
    ...
    let hits = vectors.search_with_embedding_and_language(&[1.0, 0.0], 100, None).unwrap();
...
fn body_cache_planner_matches_cold_warm_edit_and_deleted_parent() {
    ...
    workspace.configure(&endpoint, "fixture-model", 2, "code_representation = \"symbol_body_v1\"");
    ...
    workspace.rebuild();
    sent(&endpoint, 0);
    assert_ne!(before_ids, current_ids(&workspace, &["alpha_owner", "helper"]));
...
fn body_cache_policy_switch_and_corruption_never_reuse_legacy_vectors() {
    ...
    sent(&endpoint, 1);
    workspace.configure(&endpoint, "fixture-model", 2, "code_representation = \"symbol_body_v1\"");
    std::fs::write(workspace.root().join(".codanna/index/semantic/embedding-cache.json"), "corrupt").unwrap();
    assert_eq!(plan(&workspace, &endpoint)["snapshot_hit_inputs"], 0);
    workspace.rebuild();
    sent(&endpoint, 2);
}

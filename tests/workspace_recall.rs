...
async fn recall(home: &Path, root: &Path, args: &[&str]) -> std::process::Output {
    tokio::time::timeout(Duration::from_secs(10),
        command(env!("CARGO_BIN_EXE_codanna-recall"), home, root).args(args).output()
    ).await.unwrap().unwrap()
}
...
async fn hardening_workspace_recall_import_and_mcp_share_scope_without_names_or_environment() {
    ...
    for (root, provider, marker) in [(&a, "codex", "ONLY_HISTORY_A"), (&b, "claude", "ONLY_HISTORY_B")] {
        ...
        let imported = recall(&home, root, &["import", "--provider", provider, "--file", file.to_str().unwrap()]).await;
        assert!(imported.status.success(), "{}", String::from_utf8_lossy(&imported.stderr));
        assert!(!root.join(".codanna").exists(), "import must not initialize the code index");
    }
    ...
    for (root, expected, forbidden) in [(&a, "ONLY_HISTORY_A", "ONLY_HISTORY_B"), (&b, "ONLY_HISTORY_B", "ONLY_HISTORY_A")] {
        let mut process = command(env!("CARGO_BIN_EXE_codanna"), &home, root);
        process.args(["workspace", "serve"])
            .env("CODANNA_RECALL_WORKSPACE", "wrong-inherited-namespace")
            .env("CODANNA_RECALL_INDEX", temp.path().join("wrong-index"))
            .stderr(Stdio::null());
        let client = tokio::time::timeout(Duration::from_secs(15),
        ...
        let result = tokio::time::timeout(Duration::from_secs(60), async {
            loop {
                ...
                if result.structured_content.as_ref().is_none_or(|value| value["result"]["ready"] != false) {

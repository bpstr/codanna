...
impl Fixture {
    fn new() -> Self {
        ...
        for (path, name, description) in [
            ("active/calendar/presentation.ts", "useAccountPresentation", "active account first day preferences"),
            ("active/calendar/Calendar.ts", "Calendar", "active component"),
            ("reference/calendar/Calendar.ts", "Calendar", "reference implementation"),
            ("active/calendar-old/decoy.ts", "oldCalendar", "sibling prefix decoy"),
            ("active/calendar/Calendar.ts.backup.ts", "backupCalendar", "file prefix decoy"),
        ] {
            write_ts(&root, path, &format!(
                "/** Calendar settings {description}. */\nexport function {name}() {{ return 1; }}\n"
            ));
        }
        for i in 0..96 {
            write_ts(&root, &format!("archive/calendar/settings_{i:02}.ts"), &format!(
    ...
    fn scoped(&self, prefix: &str, limit: usize) -> Vec<SearchResult> {
        self.index.search_scoped(
            "calendar settings", limit, None, None, Some("typescript"), Some(prefix),
        ).unwrap()
    }
}
...
fn subtree_scope_precedes_top_k_and_broad_search_keeps_reference_code() {
    ...
    let broad = fixture.index.search("calendar settings", 200, None, None, Some("typescript")).unwrap();
    assert!(broad.iter().any(|hit| hit.file_path.starts_with("src/archive/")));
    assert!(broad.iter().any(|hit| hit.file_path.starts_with("src/reference/")));
...
fn exact_file_scope_and_subtree_boundaries_exclude_prefix_neighbors() {
    ...
    assert!(directory.iter().all(|hit| hit.file_path.starts_with("src/active/calendar/")));
    assert!(!directory.iter().any(|hit| hit.name == "oldCalendar"));
    let aliases = fixture.scoped("./src//active/./calendar/", 10);
    assert_eq!(
        directory.iter().map(|hit| hit.symbol_id).collect::<Vec<_>>(),
...
fn root_scope_matches_unscoped_results_and_escape_paths_are_rejected() {
    let fixture = Fixture::new();
    let broad = fixture.index.search("calendar settings", 5, None, None, Some("typescript")).unwrap();
    let root = fixture.scoped(".", 5);
    assert_eq!(
        broad.iter().map(|hit| hit.symbol_id).collect::<Vec<_>>(),
        root.iter().map(|hit| hit.symbol_id).collect::<Vec<_>>()
    );
    for invalid in ["", " ", "../reference", "src/../reference", "/src", r"C:\src", r"\\server\share"] {
        let error = fixture.index.search_scoped(
            "calendar settings", 5, None, None, Some("typescript"), Some(invalid),
        ).unwrap_err();
        assert!(matches!(
            error,
...
fn exact_same_name_lookup_and_language_filters_remain_independent() {
    ...
    assert!(calendars.iter().any(|symbol| symbol.file_path.as_ref() == "src/active/calendar/Calendar.ts"));
    assert!(calendars.iter().any(|symbol| symbol.file_path.as_ref() == "src/reference/calendar/Calendar.ts"));
    assert!(fixture.index.search_scoped(
        "calendar settings", 10, None, None, Some("rust"), Some(ACTIVE),
    ).unwrap().is_empty());
}
...
fn scope_tracks_reindex_deletion_and_reopen_without_global_fallback() {
    ...
    fixture.index.index_directory(&fixture.temp.path().join("src"), false).unwrap();
    assert!(fixture.scoped(REFERENCE, 5).is_empty());

    write_ts(fixture.temp.path(), relative,
    ...
    let results = reopened.search_scoped(
        "calendar settings", 5, None, None, None, Some(REFERENCE),
    ).unwrap();
    assert_eq!(results.len(), 1);
    assert_eq!(results[0].name, "replacementCalendar");
}

...
fn assert_only_restored_use(source: &str, target: &str) {
    ...
    assert_eq!(
        uses.len(),
        1,
        "only the out-of-loop binding is known: {source}\n{uses:?}"
    );
...
fn target(index: &IndexFacade, file: &str, name: &str) -> Symbol {
    let matches: Vec<_> = index
        ...
        .into_iter()
        .filter(|symbol| Path::new(symbol.file_path.as_ref()).ends_with(file))
        .collect();
    ...
    let check = |index: &IndexFacade, loop_uses_provider: bool| {
        ...
        for (name, expected) in [("Loop", usize::from(loop_uses_provider)), ("Real", 1)] {
            ...
            for (callee, _) in edges {
                assert_eq!(callee.id, provider.id);
            }
            assert!(
                !index
                    .get_called_functions(owner.id)
                    .iter()
                    .any(|callee| callee.name.as_ref() == "Calendar")
            );
        }
    };
    check(&index, false);

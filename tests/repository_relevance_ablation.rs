...
fn identifier_words(text: &str) -> String {
    ...
    for (position, &character) in chars.iter().enumerate() {
        if position > 0 && character.is_uppercase() {
            ...
            let next_lower = chars.get(position + 1).is_some_and(|next| next.is_lowercase());
...
fn coverage(row: &SearchResult, query: &BTreeSet<String>, stem: bool, paths: bool) -> usize {
    ...
    for field in [row.doc_comment.as_deref(), row.signature.as_deref(), row.context.as_deref()]
        .into_iter().flatten()
    {
...
fn ordered_ids(pool: &[SearchResult], query: &str, mode: &str) -> Vec<SymbolId> {
    ...
    let mut rows: Vec<_> = pool.iter().map(|row| {
        let score = if mode == "raw" { 0 } else { coverage(row, &terms, stem, mode.ends_with("paths")) };
        (row, score)
    }).collect();
...
fn diversity_control(pool: &[SearchResult]) -> Vec<SymbolId> {
    ...
    for row in pool {
        ...
        if *count < 2 { front.push(row.symbol_id); } else { deferred.push(row.symbol_id); }
        *count += 1;
    }
...
fn rank(ids: &[SymbolId], owner: SymbolId) -> Option<usize> {
    ids.iter().position(|id| *id == owner).map(|position| position + 1)
}
...
fn compare_bounded_relevance_alternatives_without_relabeling_misses() {
    ...
    println!("recovery_symbols={}", serde_json::to_string(&symbols).unwrap());
    ...
    for symbol in &symbols {
        let edges = match index.graph_neighbors(symbol.id, RelationKind::Calls, false, Some(32)) {
            Ok(neighbors) => json!({"status":"complete_indexed_neighbors", "targets":neighbors.iter().map(|(target, _)| target.id.value()).collect::<Vec<_>>() }),
            Err(error) => json!({"status":"unavailable_or_budget_exceeded", "error":error.to_string()}),
        };
        println!("recovery_graph={}", json!({"source":symbol.id.value(),"calls":edges}));
    }
    let mut measured = 0;
    for task in oracle["tasks"].as_array().unwrap() {
        let name = task["name"].as_str().unwrap();
        let path = task["path"].as_str().unwrap();
        let owners: Vec<_> = symbols.iter().filter(|symbol| symbol.name.as_ref() == name && symbol.file_path.as_ref() == path).collect();
        ...
        for (family, query) in task["queries"].as_array().unwrap().iter().enumerate() {
            ...
            variants.insert("current", rank(&baseline.iter().map(|row| row.symbol_id).collect::<Vec<_>>(), owner));
...
fn token_controls_distinguish_substrings_without_losing_identifier_parts() {
    ...
    assert!(tokens("HTTPServer read_file", false).is_superset(&BTreeSet::from(["http".into(), "server".into(), "read".into(), "file".into()])));
    assert_eq!(tokens("vector vectors", true), BTreeSet::from(["vector".into()]));
}

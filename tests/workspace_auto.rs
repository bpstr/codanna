...
fn cli(home: &Path, cwd: &Path, args: &[&str]) -> Output {
    ...
    command.env_clear().env("HOME", home).env("PATH", "/usr/bin:/bin")
        .env("NO_COLOR", "1").current_dir(cwd).args(args);
...
fn hardening_workspace_auto_discovery_has_no_side_effects() {
    ...
    let result: Value = serde_json::from_str(&ok(cli(&home, &root, &["workspace", "discover", "--json"]))).unwrap();
...
fn hardening_workspace_auto_assign_and_codanna_index_without_registration_commands() {
    ...
    ok(cli(&home, &codanna.join("src"), &["index", "--no-progress"]));
    let registrations: Value = serde_json::from_str(&ok(cli(&home, &assign, &["workspace", "list", "--json"]))).unwrap();
    assert_eq!(registrations["workspaces"].as_array().unwrap().len(), 2);
    let results = ok(cli(&home, &web.join("src"), &["retrieve", "search", "product_unique_identity", "--json"]));
    let results: Value = serde_json::from_str(&results).unwrap();
    let items = results["data"]["items"].as_array().unwrap();
    assert_eq!(items.len(), 2, "Assign query should include both member repositories: {results}");
    ...
    let unrelated = ok(cli(&home, &codanna, &["retrieve", "search", "unrelated_unique_identity", "--json"]));
    assert!(unrelated.contains("unrelated_unique_identity"));
    assert!(!unrelated.contains("product_unique_identity"));
    assert_eq!(fs::read(web.join(".codanna/settings.toml")).unwrap(), child_config);
...
fn hardening_workspace_auto_bare_index_fills_only_fresh_empty_source_list() {
    ...
    let query = ok(cli(&home, &root, &["retrieve", "search", "initialized_project_identity", "--json"]));
...
fn hardening_workspace_auto_inspect_from_sibling_does_not_create_product_parent() {
    ...
    for root in [&assign, &codanna] {
        let result: Value = serde_json::from_str(&ok(cli(&home, &root.join("src"), &["workspace", "discover", "--json"]))).unwrap();
        assert_eq!(result["root"], root.canonicalize().unwrap().to_string_lossy().as_ref());
    }

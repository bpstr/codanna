...
fn rank(items: &Value, name: &str, path: &str) -> Option<usize> {
    items.as_array().unwrap().iter().position(|item| item["name"] == name && item["file_path"] == path).map(|position| position + 1)
}
...
async fn ticket_related_relevance_preserves_direct_results_and_measures_related_owners() {
    ...
    let mut settings = Settings { workspace_root: Some(root.clone()), index_path: temp.path().join("index"), ..Default::default() };
    ...
    for task in oracle["tasks"].as_array().unwrap() {
        ...
        assert_eq!(matches.iter().filter(|symbol| symbol.file_path.as_ref() == task["path"].as_str().unwrap()).count(), 1);
    }
    ...
    for task in oracle["tasks"].as_array().unwrap() {
        ...
        for (family, query) in task["queries"].as_array().unwrap().iter().enumerate() {
            ...
            let response = server.search_ticket_context(Parameters(original)).await.unwrap();
            assert_ne!(response.is_error, Some(true));
            let direct = response.structured_content.unwrap();
            let mut opted_in = value;
            opted_in["include_related_code"] = json!(true);
            let response = server.search_ticket_context(Parameters(serde_json::from_value(opted_in).unwrap())).await.unwrap();
            assert_ne!(response.is_error, Some(true));
            let expanded = response.structured_content.unwrap();
            assert_eq!(expanded["code"]["items"], direct["code"]["items"], "opting in must not reorder or rescore direct candidates");
            ...
            println!("ticket_related_case={}", json!({
            ...
            if family == 0 && matches!(task["id"].as_str(), Some("recall-timeout" | "foreign-recall")) {
                assert!(related_rank.is_some(), "indexed call witness should recover {name} as related evidence");
            }
            queries += 1;
        }
    }
    assert_eq!(queries, 20);
    println!("ticket_related_summary={}", json!({

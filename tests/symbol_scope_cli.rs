...
impl Workspace {
    fn new() -> Self {
        ...
        for (directory, name) in [
            ...
            std::fs::write(
                source.join("calendar.rs"),
                format!("/// Calendar settings for {directory}.\npub fn {name}() {{}}\n"),
            )
            .unwrap();
        }
...
fn envelope(output: &Output) -> Value {
    serde_json::from_slice(&output.stdout)
        .unwrap_or_else(|error| panic!("invalid envelope: {error}; {}", diagnostics(output)))
}
...
fn malformed_scopes_fail_instead_of_becoming_global_searches_or_success_envelopes() {
    ...
    for (tool, field) in [
        ...
        ("search_context", "code_path_prefix"),
    ] {
        for invalid in [
            json!("../outside"),
            json!(""),
